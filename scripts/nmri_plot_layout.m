function nmri_plot_layout(lay, varargin)
...
% added
bad_channels    = ft_getopt(varargin, 'bad_channels');
channel_hit_callback    = ft_getopt(varargin, 'channel_hit_callback');
elec_color    = ft_getopt(varargin, 'elec_color');
elec_bold    = ft_getopt(varargin, 'elec_bold');


% these have to do with the font
fontcolor   = ft_getopt(varargin, 'fontcolor', 'k'); % default is black
fontsize    = ft_getopt(varargin, 'fontsize',   8);
...
% default color 
if isempty(elec_color) || length(elec_color)~=length(lay.label)
 elec_color=repmat({fontcolor},length(lay.label),1);
end
% default bold 
if isempty(elec_bold) || length(elec_bold)~=length(lay.label)
 elec_bold=repmat({fontweight},length(lay.label),1);
end
...
if point
  if ~isempty(pointsymbol) && ~isempty(pointcolor) && ~isempty(pointsize) % if they're all non-empty, don't use the default
    plot(X, Y, 'marker', pointsymbol, 'color', pointcolor, 'markersize', pointsize, 'linestyle', 'none','HitTest','Off');
  else
    plot(X, Y, 'marker', '.', 'color', 'b', 'linestyle', 'none','HitTest','Off');
    plot(X, Y, 'marker', 'o', 'color', 'y', 'linestyle', 'none','HitTest','Off');
  end
end
...
if label
  ...
  if numel(labelrotate)==1
   % loop for each label
   for i=1:length(Lbl)
    if isempty(channel_hit_callback)
     text(X(i)+labelxoffset, Y(i)+labelyoffset, Lbl{i} , 'interpreter', interpreter, 'horizontalalignment', labelalignh, 'verticalalignment', labelalignv, 'color', elec_color{i}, 'fontunits', fontunits, 'fontsize', fontsize, 'fontname', fontname, 'fontweight', elec_bold{i});
    else
     text(X(i)+labelxoffset, Y(i)+labelyoffset, Lbl{i}, 'interpreter', interpreter, 'horizontalalignment', labelalignh, 'verticalalignment', labelalignv, 'color', elec_color{i}, 'fontunits', fontunits, 'fontsize', fontsize, 'fontname', fontname, 'fontweight', elec_bold{i},...
      'ButtonDownFcn',channel_hit_callback);
    end
   end 
  else
    n = numel(Lbl);
...
if box
  line([X-Width/2 X+Width/2 X+Width/2 X-Width/2 X-Width/2]',[Y-Height/2 Y-Height/2 Y+Height/2 Y+Height/2 Y-Height/2]', 'color', [0 0 0],'HitTest','Off');
end
